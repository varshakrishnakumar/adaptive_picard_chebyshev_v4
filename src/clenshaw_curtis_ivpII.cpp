...
void clenshaw_curtis_ivpII( int N, int M, double* T2, double* P2, double* T1, double* P1, double* Ta, double* A ){
  ...
  double Lv[N*N];
  std::memset( Lv, 0.0, ((N*N)*sizeof(double)));
  for (int k=0; k<=N-1; k++){
  ...
  double Lp[(N+1)*(N+1)];
  std::memset( Lp, 0.0, ((N+1)*(N+1)*sizeof(double)));
  for (int k=0; k<=N; k++){
  ...
  double temp1[N*N];
  std::memset( temp1, 0.0, ((N*N)*sizeof(double)));
  temp1[ID2(1,1,N)] = 1.0;
  for (int i=1; i<N; i++){
    for (int j=1; j<N; j++){
      if (i == j){
  ...
  std::memset( temp2, 0.0, ((N-1)*(N-1)*sizeof(double)));
  for (int i=1; i<=N-1; i++){
    for (int j=1; j<=N-1; j++){
      if (i == j){
  ...
  std::memset( temp3, 0.0, ((N-1)*(N-1)*sizeof(double)));
  for (int i=1; i<=N-1; i++){
    for (int j=3; j<=N-1; j++){
      if (i == j-2){
  ...
  double temp4[N*(N-1)];
  std::memset( temp4, 0.0, (N*(N-1)*sizeof(double)));
  for (int i=2; i<=N; i++){
    for (int j=1; j<=N-1; j++){
  ...
  double Sv[N*(N-1)];
  std::memset( Sv, 0.0, (N*(N-1)*sizeof(double)));
  ...
  double temp5[N*N];
  std::memset( temp5, 0.0, (N*N*sizeof(double)));
  for (int i=1; i<=N; i++){
    for (int j=1; j<=N; j++){
  ...
  std::memset( temp6, 0.0, ((N+1)*(N+1)*sizeof(double)));
  ...
  double temp7[(N)*(N)];
  std::memset( temp7, 0.0, ((N)*(N)*sizeof(double)));
  for (int i=1; i<=N; i++){
    for (int j=1; j<=N; j++){
      if (i == j){
        temp7[ID2(i,j,N)] = 1.0;
  ...
  double temp8[(N)*(N)];
  std::memset( temp8, 0.0, ((N)*(N)*sizeof(double)));
  for (int i=1; i<=N; i++){
    for (int j=3; j<=N; j++){
      if (i == j-2){
        temp8[ID2(i,j,N)] = -1.0;
  ...
  double temp9[(N+1)*N];
  std::memset( temp9, 0.0, ((N+1)*N*sizeof(double)));
  for (int i=2; i<=N+1; i++){
    for (int j=1; j<=N; j++){
  ...
  double Sp[(N+1)*N];
  std::memset( Sp, 0.0, ((N+1)*N*sizeof(double)));
  ...
  std::memset( temp10, 0.0, ((N+1)*(N+1)*sizeof(double)));
  for (int i=1; i<=N+1; i++){
    for (int j=1; j<=N+1; j++){
